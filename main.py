...
def is_henan_unicom(url):
    """判断是否为河南联通源"""
    return any(keyword in url.lower() for keyword in ['ha.10010.cn', 'henan.unicom', '河南联通', 'unicom'])

def filter_henan_unicom_urls(urls):
    """筛选河南联通的前4个源"""
    henan_unicom_urls = [url for url in urls if is_henan_unicom(url)]
    return henan_unicom_urls[:4]  # 只保留前4个河南联通源
